...
export const getBoards = (token: string) => async (dispatch: AppDispatch): Promise<void> => {
    dispatch(fetchBoardsStart());
    try {
        const { status, data } = await axios.get<Board<string>[]>('/api/boards', getAuthHeader(token));
...
export const getBoard = (token: string, boardId: string) => async (dispatch: AppDispatch): Promise<void> => {
    dispatch(fetchBoardStart());
    try {
        const { status, data } = await axios.get<Board<List>>('/api/boards/' + boardId, getAuthHeader(token));
        if (!isResponseOk(status)) {
            throw new Error(data.toString());
        }
        // TODO init lists 
...
export const deleteBoard = (token: string, boardId: string) => async (dispatch: AppDispatch): Promise<void> => {
    dispatch(deleteBoardStart());
    try {
        const { status, data } = await axios.delete<Board<List>>('/api/boards/' + boardId, getAuthHeader(token));
        if (!isResponseOk(status)) {
            throw new Error(data.toString());
        }
...
export const createBoard = (token: string, payload: BoardPayload<'name' | 'owner' | 'theme'>) => async (
    dispatch: AppDispatch
): Promise<void> => {
    dispatch(createBoardStart());
    try {
        const { status, data } = await axios.post<Board<string>>('/api/boards', payload, getAuthHeader(token));
        if (!isResponseOk(status)) {
            throw new Error(data.toString());
        }
...
export const updateBoard = (token: string, boardId: string, payload: BoardPayload<'name' | 'theme'>) => async (
    dispatch: AppDispatch
): Promise<void> => {
    dispatch(updateBoardStart());
    try {
        const { status, data } = await axios.patch<Board<List>>(
            '/api/boards/' + boardId,
            payload,
            getAuthHeader(token)
        );
